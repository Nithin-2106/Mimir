...
function MangaRow({ manga, index, onNavigate }) {
  ...
  const goToInfo = () => {
    if (manga.anilistId) onNavigate('Info', manga.anilistId)
  }
  ...
  return (
    ...
    >
      ...
      <td style={{ ...tdBase, padding: '14px 18px', width: '150px' }}>
        <span style={{
          fontSize: '13px',
          color: manga.dateCompleted ? C.textMuted : C.textDim,
          fontFamily: '"Cinzel", serif',
...
function StatusTab({ label, count, active, onClick }) {
  ...
  const activeColor = isAll ? C.primary : color
...
export default function MyList({ onNavigate }) {
  ...
  const counts = useMemo(() => {
    ...
    STATUS_TABS.slice(1).forEach(s => {
      map[s] = manga.filter(m => m.status === s).length
    })
    return map
  }, [manga])
